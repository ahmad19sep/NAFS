...
// getSession()/getUser() here triggers an untimed token-refresh fetch on every
// expired cookie, which hangs the edge function until Vercel kills it
// (MIDDLEWARE_INVOCATION_TIMEOUT). Cookie presence is enough: this is only a
// redirect gate, and every page/route re-verifies via requireUser()/getUser().
