...
// Light theme on purpose — dark backgrounds waste ink and most printers drop
// them anyway. Rendered into an iframe (or a new window) and sent to print.

import { prettyDate, type ReportData } from '@/lib/report'
...
export function reportToHtml(d: ReportData, opts: { autoPrint?: boolean } = {}): string {
  ...
  const salah = !d.deenEnabled || d.prayers_possible === 0 ? '' : `
    ...
    ${d.extra_prayers.length === 0 ? '' : `
      <h3>Extra / nafl</h3>
      <table>
        <thead><tr><th>Prayer</th><th>Done</th><th class="w-40">Rate</th></tr></thead>
        <tbody>
          ${d.extra_prayers.map(x => `
            <tr>
              <td><strong>${esc(x.name)}</strong></td>
              <td>${x.done}/${x.days}</td>
              <td>${bar(x.pct, scoreColor(x.pct))}<span class="pct">${x.pct}%</span></td>
            </tr>`).join('')}
        </tbody>
      </table>`}
    <p class="note">
      ...
      <tbody>
        ${d.habits.map(h => `
          <tr>
            ...
            <td>${h.done_days}/${h.scheduled_days}</td>
      ...
      <tbody>
        ${d.tasks.map(t => `
          <tr>
            ...
            <td>${t.completed}</td><td>${t.total}</td>
            <td>${bar(t.pct, scoreColor(t.pct))}<span class="pct">${t.pct}%</span></td>
          </tr>`).join('')}
      </tbody>
      <tfoot><tr><td><strong>All tasks</strong></td><td><strong>${d.tasks_completed}</strong></td><td><strong>${d.tasks_total}</strong></td><td><strong>${d.tasks_pct}%</strong></td></tr></tfoot>
    </table>
  ...
  const goals = d.goals.length === 0 ? '' : `
    <table>
      <thead><tr><th>Goal</th><th class="w-40">Progress</th><th>Milestones</th><th>Alignment</th><th>Deadline</th></tr></thead>
      <tbody>
        ${d.goals.map(g => `
          <tr>
            ...
            <td>${g.alignment == null ? '—' : `${g.alignment}%`}</td>
            <td>${g.deadline
  ...
  const health = h.days_logged === 0 ? '' : `
    ...
    ${h.metrics.length === 0 ? '' : `
      ...
      <table>
        <thead><tr><th>Metric</th><th>Hit target</th><th class="w-40">Rate</th></tr></thead>
        <tbody>
          ${h.metrics.map(m => `
            <tr>
              <td>${esc(m.emoji)} <strong>${esc(m.name)}</strong></td>
              <td>${m.done}/${m.days}</td>
              <td>${bar(m.pct, scoreColor(m.pct))}<span class="pct">${m.pct}%</span></td>
            </tr>`).join('')}
        </tbody>
      </table>`}`
  ...
  const dailyLog = d.days.length === 0 ? '' : `
    <table class="dense">
      <thead>
        <tr>
          <th>Date</th><th>Score</th>${d.deenEnabled ? '<th>Salah</th><th>Jamaat</th><th>Quran</th>' : ''}
          <th>Habits</th><th>Tasks</th><th>Sleep</th><th>Steps</th><th>Water</th><th>Exercise</th><th>Mood</th>
        </tr>
      </thead>
      <tbody>
        ${d.days.map(day => `
          <tr class="${day.logged ? '' : 'empty-row'}">
            ...
            ${d.deenEnabled ? `<td>${day.prayers_prayed}/5</td><td>${day.prayers_jamat}</td><td>${day.quran_pages || '—'}</td>` : ''}
...
<style>
  ...
  * { box-sizing: border-box; }
  body {
    ...
    color: #12212f; margin: 0; padding: 10px; font-size: 11px; line-height: 1.5; background: #fff;
  ...
  .kpi { flex: 1 1 100px; min-width: 100px; border: 1px solid #e3e8ee; border-radius: 8px;
  ...
  h2 { font-size: 13px; font-weight: 800; color: ${NAVY}; margin: 0 0 8px;
  ...
  .quote-verdict { color: #64748b; font-size: 10px; margin-top: 3px; }
...
<div class="kpi-row">
  ...
  ${d.deenEnabled ? kpi('Salah', `${d.prayers_prayed}/${d.prayers_possible}`, `${d.prayers_jamat} in jamaat`) : ''}
